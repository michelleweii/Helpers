...
def load_zip_file(file,fileNameRegExp='',allEntries=False):
    ...
    pairs = []
    # print([item.split('/')[-1] for item in archive.namelist() if item != ''])
    for name in archive.namelist():
    # for name in [item.split('/')[-1] for item in archive.namelist() if item.endswith('txt')]:
        addFile = True
        keyName = name
        ...
        t_name = name.split('/')[-1]
        if fileNameRegExp!="":
            # print(fileNameRegExp,t_name)
            m = re.match(fileNameRegExp,t_name)
            ...
            if m == None:
                ...
                if len(m.groups())>0:
                    keyName = m.group(1)
        # print([keyName,archive.read(name)])
        if addFile:
            pairs.append([keyName,archive.read(name)])
        else:
            if allEntries:
                raise Exception('ZIPP entry not valid: %s' %name)
    # print(dict(pairs))
    return dict(pairs)
